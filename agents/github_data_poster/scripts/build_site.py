#!/usr/bin/env python3
# ==========================================================================
# build_site.py — Static HTML site generator (gitfolio-inspired, light theme)
...
def build_home(groups_data: list[dict]) -> str:
    ...
    for g in groups_data:
        ...
        cards.append(
            f'<a class="group-card" href="/blog/{g["slug"]}/">'
            f'<h2>{html_mod.escape(g["name"])}</h2>'
            f'<p class="card-desc">{html_mod.escape(g["desc"])}</p>'
            f'<div class="card-meta">'
            f'<span class="badge">{cat_count} categories</span>'
            f'<span>{total} posts</span>'
            f'</div></a>'
        )
    return (

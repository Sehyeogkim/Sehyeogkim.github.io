...
sidebar:
  nav: "docs"
source_url: "https://jeffdissel.tistory.com/220"
source_id: "220"
date: "2025-09-04T22:26:40+09:00"
category: "Computational-Linear-Algebra"
---

Source: [https://jeffdissel.tistory.com/220](https://jeffdissel.tistory.com/220)

자 지난시간에는 아주 빠르게 Linear Algebra를 한번 쭉 살펴보았고,
이제 진짜 computer을 이용해서 Matrix Eq을 어떻게 효과적으로
구할 수 있을지에 대해서 살펴보자.
그 전에 projector Matrix를 정의하고 넘어가자.
![2. QR Factorization - part1 (Gram Schmedit)](./images/img-001.png)
1. Matrix P는 자기자신을 아무리 곱해도 자기자신이 나온다.
![2. QR Factorization - part1 (Gram Schmedit)](./images/img-002.png)
2. P의 열벡터의 선형결합으로 이루어진 벡터 v를
우리는 당연히 v = Px라고 할 수 있다.
![2. QR Factorization - part1 (Gram Schmedit)](./images/img-003.png)
신기하게 v = Px -> Pv = v
라는 아주 신기한 성질을 1번 성질로 유도가능.
만약에 v와 Pv가 다르다면??
Pv - v = 벡터 b라고 정의하면,
그렇다면, P b = 0 이 나오게 된다
![2. QR Factorization - part1 (Gram Schmedit)](./images/img-004.png)
...
![2. QR Factorization - part1 (Gram Schmedit)](./images/img-005.png)
즉 P는 range(P) 공간을 벡터를 mapping하는 matrix라는 것.
![2. QR Factorization - part1 (Gram Schmedit)](./images/img-006.png)
여기서 한발짝 더 나아가서 orthogonal Projector는 그중에서도,
P = P* (transpose)성질을 만족하는 Projector라고 정의하자.
![2. QR Factorization - part1 (Gram Schmedit)](./images/img-007.png)
range(P) = v = Px
null(P) = (I-P)v
두 벡터를 내적하면 (P = P*이므로) 0 이나옴을 알 수 있다.
![2. QR Factorization - part1 (Gram Schmedit)](./images/img-008.png)
결국, range(P)와 null(P)는 orthogonal 하다라고 할 수 있다.
![2. QR Factorization - part1 (Gram Schmedit)](./images/img-009.png)
여기서 Orthogonal Projector P 를 쪼갤 것이다.
P = Q QT 로 쪼개고 Q는 Orthonormal vectors
(orthogonal each other with a unit length)
![2. QR Factorization - part1 (Gram Schmedit)](./images/img-010.png)
자 이제 위 projector 개념을 이용해서, A로의 projection을 살펴보자.
![2. QR Factorization - part1 (Gram Schmedit)](./images/img-011.png)
임의의 벡터 v를 우리가 range(A) 로 projection한 벡터를 y라고 하자.
그렇다면, v - y 는 range(A)와 정의에 따라 수직 일 것이다.
![2. QR Factorization - part1 (Gram Schmedit)](./images/img-012.png)
...
y = P v = (A) (A*A)-1 A* v
따라서,
![2. QR Factorization - part1 (Gram Schmedit)](./images/img-013.png)
이후에 쓰이는 householder reflector개념도 짚고 넘어가자.
![2. QR Factorization - part1 (Gram Schmedit)](./images/img-014.png)
![2. QR Factorization - part1 (Gram Schmedit)](./images/img-015.png)
![2. QR Factorization - part1 (Gram Schmedit)](./images/img-014.png)
...
(추후에 쓰임, 그냥 정의만 알아두자)
![2. QR Factorization - part1 (Gram Schmedit)](./images/img-016.png)
...
![2. QR Factorization - part1 (Gram Schmedit)](./images/img-017.png)
A의
열벡터들을 확장하여 새로운 space를 구성
한다는 의미가 숨어져 있다.
A의 열벡터들을 가지고 우리가 Orthonormal vectors q1 q2 .... 를 제작하였다고 가정하자.
![2. QR Factorization - part1 (Gram Schmedit)](./images/img-018.png)
...
![2. QR Factorization - part1 (Gram Schmedit)](./images/img-019.png)
가장 간단하게 A 를 안다고 가정하고 Q,R을 구하는 기본적인 방식은
Gram - Schmidt Algorithm .
![2. QR Factorization - part1 (Gram Schmedit)](./images/img-020.png)
예시로 정확히 이해해보자. 다음과 같이 A (3 x 2)행렬이 있다고 하자.
![2. QR Factorization - part1 (Gram Schmedit)](./images/img-021.png)
먼저 우리는 첫번째 직교벡터 q1을 구할수 있고, 그 계수 r11도 도출가능하다.
![2. QR Factorization - part1 (Gram Schmedit)](./images/img-022.png)
두번째 과정은 a2에서 v1성분을 제거하고 normalize하여 새로운 v2를 정의하는 것.
![2. QR Factorization - part1 (Gram Schmedit)](./images/img-023.png)
j = 2
And normalize this.
![2. QR Factorization - part1 (Gram Schmedit)](./images/img-024.png)
따라서, 최종적응로, 우리는 Q,R행렬을 다음과 같이 유도가능하다.
![2. QR Factorization - part1 (Gram Schmedit)](./images/img-025.png)
...
![2. QR Factorization - part1 (Gram Schmedit)](./images/img-026.png)
그렇다면 QR factorization을 도대체 어디다 쓰는가??
우리의 원초적인 목표는 Matrix Equation을 푸는 게 목표였다.
![2. QR Factorization - part1 (Gram Schmedit)](./images/img-027.png)
Ax = b에서, A를 QR로 쪼깬다면,
![2. QR Factorization - part1 (Gram Schmedit)](./images/img-028.png)

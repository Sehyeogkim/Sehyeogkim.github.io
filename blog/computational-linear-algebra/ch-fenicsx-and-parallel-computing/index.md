...
sidebar:
  nav: "docs"
source_url: "https://jeffdissel.tistory.com/m/245"
source_id: "245"
date: "2025-10-30T12:20:32+09:00"
category: "Computational-Linear-Algebra"
---

Source: [https://jeffdissel.tistory.com/m/245](https://jeffdissel.tistory.com/m/245)
...
![ch (?) Fenicsx and Parallel Computing](./images/img-001.png)
...
전체 workflow는 다음과 같다.
![ch (?) Fenicsx and Parallel Computing](./images/img-002.png)
...
![ch (?) Fenicsx and Parallel Computing](./images/img-003.png)
(github에 들어가게 되면, 어떻게 사용하는지 class, method전부 설명이 되어있고,
오늘은 사용방법보다는 어떤식으로 구동되는지에 집중을 해보자)
총 process를 diagram으로 그려보면 다음과 같다.
하나씩 프로세스를 이해해보자.
![ch (?) Fenicsx and Parallel Computing](./images/img-004.jpg)
total process of fenics
1. User make python code and mesh
![ch (?) Fenicsx and Parallel Computing](./images/img-005.png)
User가 먼저 fenics에서 제공한 문법에 맞게 어떻게 문제를 풀것인지 코드를 작성한다.
(오늘은 python 코드 내용을 어떻게 구현해야하는 지는 다루지 않을것)
![ch (?) Fenicsx and Parallel Computing](./images/img-006.png)
그리고 풀문제의 MESH를 xdmf or .msh형식으로 전달한다.
![ch (?) Fenicsx and Parallel Computing](./images/img-007.png)
example of a mesh
![ch (?) Fenicsx and Parallel Computing](./images/img-008.png)
inner tetrahedrone of inner mesh
여기서핵심은, UFL이 우리가 정말
수학적 문법
에 맞게 python 코드를 작성할 수 있는 tool이라는 점이다.
![ch (?) Fenicsx and Parallel Computing](./images/img-009.png)
Example of Linear elastic Weak form Ufl equation.
![ch (?) Fenicsx and Parallel Computing](./images/img-010.png)
...
![ch (?) Fenicsx and Parallel Computing](./images/img-011.png)
...
![ch (?) Fenicsx and Parallel Computing](./images/img-012.png)
...
![ch (?) Fenicsx and Parallel Computing](./images/img-013.jpg)
(쉽게 말해 BASIX는 FEM의 핵심이 되는 기본적인 연산들을 담당한다)
3. Dolfinx 는 위의 연산 함수들과 input output을 종합해주는 즉 orchestration을 진행한다.
(가장 핵심)
![ch (?) Fenicsx and Parallel Computing](./images/img-014.png)
...
(현시점 version 0.10)
![ch (?) Fenicsx and Parallel Computing](./images/img-015.png)
...
![ch (?) Fenicsx and Parallel Computing](./images/img-016.png)
이미 굉장한 수학적? 개발자들이 연산하는 C++ package를 개발해 왔고, 그중에 PETSc를 피닉스는 사용한다.
![ch (?) Fenicsx and Parallel Computing](./images/img-017.png)
이 라이브러리의 핵심은 단순히 GMRES, PCG를 쓰는게 아니라,
병렬연산에 최적화되어 있다는 점이다.
사실 병렬연산 진행은 OPEN MPI package를 통해 실행을 한다.
![ch (?) Fenicsx and Parallel Computing](./images/img-018.png)
...
![ch (?) Fenicsx and Parallel Computing](./images/img-019.jpg)
예를들어, 우리가 아래의 fixed beam 문제를 풀때,
element 3개 에따라서 computer 3개로 나눈다고 하자.
![ch (?) Fenicsx and Parallel Computing](./images/img-020.jpg)
...
![ch (?) Fenicsx and Parallel Computing](./images/img-021.png)
5 Independent Static Problems
이때 사용되는 package는 OPEN MPI가 아닌 (햇갈리는)이름인 OPENMP
![ch (?) Fenicsx and Parallel Computing](./images/img-022.png)
![ch (?) Fenicsx and Parallel Computing](./images/img-023.jpg)
...
![ch (?) Fenicsx and Parallel Computing](./images/img-024.png)
고체 문제에서 solution x - displacement이므로,
![ch (?) Fenicsx and Parallel Computing](./images/img-025.png)
위 코드를 python에서 실행하면,
paraview에서 displacment를 visualize할 수 있게 된다.
![ch (?) Fenicsx and Parallel Computing](./images/img-026.png)

...
Source: [https://jeffdissel.tistory.com/103](https://jeffdissel.tistory.com/103)
...
![[Pulsatile flow] on a rigid tube _ 1. Velocity](./images/img-001.png)
1cycle동안 대동맥 압력 변화그래프.
참고할 책은
![[Pulsatile flow] on a rigid tube _ 1. Velocity](./images/img-002.png)
그중에 ch3,4,5를 다룰 것이고
...
로 전환시켜준다.
![[Pulsatile flow] on a rigid tube _ 1. Velocity](./images/img-003.png)
먼저 Continuity Eq
![[Pulsatile flow] on a rigid tube _ 1. Velocity](./images/img-004.png)
![[Pulsatile flow] on a rigid tube _ 1. Velocity](./images/img-005.png)
![[Pulsatile flow] on a rigid tube _ 1. Velocity](./images/img-004.png)
![[Pulsatile flow] on a rigid tube _ 1. Velocity](./images/img-005.png)
여기에 밀도 변화거 없는 incompressible flow 가정을 더해주자.
![[Pulsatile flow] on a rigid tube _ 1. Velocity](./images/img-006.png)
자 여기까지, 원통형 incompressible flow continuity Eq이다.
여기서 두가지 가정을 추가로,
Naviers Stokes eq도
간단하게 해주자.
![[Pulsatile flow] on a rigid tube _ 1. Velocity](./images/img-007.jpg)
여기서, 1번 연속방정식을 정리후,
적분해주고,
...
![[Pulsatile flow] on a rigid tube _ 1. Velocity](./images/img-008.jpg)
이후 다시, 2번째 u component
NS 방정식에 V = 0 을 대입해주면,
![[Pulsatile flow] on a rigid tube _ 1. Velocity](./images/img-009.jpg)
최종적으로, 다음의 식이 도출 된다.
이 식이 바로 우리가 분석할 맨 처음 기본 식이다.
![[Pulsatile flow] on a rigid tube _ 1. Velocity](./images/img-010.png)
(4번식)
종합해보면,
밑의 5가지 가정을 첨가하여, Naviers Stokes Eq
을 정리하였다.
![[Pulsatile flow] on a rigid tube _ 1. Velocity](./images/img-011.jpg)
이제, 위
식의 해
를 구할 것이다.
그 전에.
...
이 책의 표지를 다시 살펴보자.
![[Pulsatile flow] on a rigid tube _ 1. Velocity](./images/img-012.png)
위 사진을 살펴보면,
...
![[Pulsatile flow] on a rigid tube _ 1. Velocity](./images/img-013.png)
Steady flow의 경우 위 식을 보면 알겠지만,
시간 term이 변수로 존재하지 않는다.
즉 시간에 따라서 일정(Steady flow의 정의)
4번 식에 대입해주면,
(5번식이 당연히 성립하기 때문에 6번식도 성립하게 된다)
![[Pulsatile flow] on a rigid tube _ 1. Velocity](./images/img-014.jpg)
우리는 이제 진동 부분을 유심히 살펴볼 것이다.
이후에, Pressure Gradient 값을 다음과 같이 k를 통해 정의해주고,
똑같이 Steady, Oscillatory term을 나누어 주자.
![[Pulsatile flow] on a rigid tube _ 1. Velocity](./images/img-015.png)
![[Pulsatile flow] on a rigid tube _ 1. Velocity](./images/img-016.jpg)
새로운 정의 kpi 를 6에 대입하면,
![[Pulsatile flow] on a rigid tube _ 1. Velocity](./images/img-017.jpg)
자 이제, 압력 gradient 부분을 살펴보자.
압력은 위 밑의 형태가 주기적으로 반복된다.
![[Pulsatile flow] on a rigid tube _ 1. Velocity](./images/img-018.png)
...
![[Pulsatile flow] on a rigid tube _ 1. Velocity](./images/img-019.png)
...
![[Pulsatile flow] on a rigid tube _ 1. Velocity](./images/img-020.png)
계속 해를 구하고 있던, 7번 방정식에 대입해주고
Separation varaible로 시간과 r term을 나누어 주고 다시 대입.
![[Pulsatile flow] on a rigid tube _ 1. Velocity](./images/img-021.jpg)
여기서,
[Womersley Number 무차원수]
![[Pulsatile flow] on a rigid tube _ 1. Velocity](./images/img-022.png)
Ω
:
웸슬리 수
(Womersley number)
...
웸슬리 . 수를 대입하여 준다.
![[Pulsatile flow] on a rigid tube _ 1. Velocity](./images/img-023.jpg)
여기서 11번 식은 Non-homogeneous ODE이다.
공학수학 시간에 다루었지만,
Non-homogeneous ODE같은 경우,
![[Pulsatile flow] on a rigid tube _ 1. Velocity](./images/img-024.jpg)
...
![[Pulsatile flow] on a rigid tube _ 1. Velocity](./images/img-025.jpg)
치환한 요소들을 전부 더해주면,
밑의 식처럼 m = 0 인 베셀함수가 나오게 된다.
![[Pulsatile flow] on a rigid tube _ 1. Velocity](./images/img-026.jpg)
[블로그 베셀함수]
...
![[Pulsatile flow] on a rigid tube _ 1. Velocity](./images/img-027.jpg)
이후, 추가 상수 term을 추가해서,
원래식 11에 대입해주면, 쉽게 상수항이 도출된다.
![[Pulsatile flow] on a rigid tube _ 1. Velocity](./images/img-028.jpg)
...
![[Pulsatile flow] on a rigid tube _ 1. Velocity](./images/img-029.png)
![[Pulsatile flow] on a rigid tube _ 1. Velocity](./images/img-030.jpg)
최종적으로 정리해보면,
다음과 같이 도출된다.
![[Pulsatile flow] on a rigid tube _ 1. Velocity](./images/img-031.jpg)

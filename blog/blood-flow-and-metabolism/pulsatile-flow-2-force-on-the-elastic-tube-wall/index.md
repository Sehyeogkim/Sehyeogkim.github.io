...
Source: [https://jeffdissel.tistory.com/124](https://jeffdissel.tistory.com/124)
...
시간, x에 따른 주기성
![[Pulsatile flow] 2. Force on the elastic tube wall](./images/img-002.png)
A,B는 일반해의 임의의 상수이고,
감마는 다음과 같이 정의 하였다.
(지난포스터참고)
...
![[Pulsatile flow] 2. Force on the elastic tube wall](./images/img-004.png)
--------------
...
![[Pulsatile flow] 2. Force on the elastic tube wall](./images/img-007.png)
...
![[Pulsatile flow] 2. Force on the elastic tube wall](./images/img-009.png)
Navier stokes 방정식 증명하듯이, 튜브의 미소부피가 받는 힘을 분석해보자.
먼저 미소부피와 질량은 다음과 같이 쉽게 정의된다.
![[Pulsatile flow] 2. Force on the elastic tube wall](./images/img-010.png)
...
1. Axial direction (+x)
![[Pulsatile flow] 2. Force on the elastic tube wall](./images/img-012.png)
먼저, Axial 방향으로는 유체에 의한
전단응력과 옆의 고체 요소의 stress Sxx가 있다.
이를 뉴턴의 제2법칙 F=ma로 정리하면,
![[Pulsatile flow] 2. Force on the elastic tube wall](./images/img-013.png)
...
이 곡률로 인하여
-R방향으로(Srr) 작용하는 것이다.
![[Pulsatile flow] 2. Force on the elastic tube wall](./images/img-016.png)
따라서, Srr을
...
![[Pulsatile flow] 2. Force on the elastic tube wall](./images/img-018.png)
...
![[Pulsatile flow] 2. Force on the elastic tube wall](./images/img-020.png)
...
![[Pulsatile flow] 2. Force on the elastic tube wall](./images/img-024.png)
...
![[Pulsatile flow] 2. Force on the elastic tube wall](./images/img-027.png)
결론적으로 1번 효과는 존재하지 않고,
2번 효과로 인한 angular strain을 유도해주면 7번식이 도출된다.
![[Pulsatile flow] 2. Force on the elastic tube wall](./images/img-028.png)
우리가 방금전에 구한 Strain들을, 4,5번 식
(generalized hook's law)에 대입해주면
strain과 stress의 관계식 8,9가 도출된다.
![[Pulsatile flow] 2. Force on the elastic tube wall](./images/img-029.png)
자 우리가 여기까지 구한 이유는 맨처음에 구한 1,2 Force식에서
![[Pulsatile flow] 2. Force on the elastic tube wall](./images/img-030.png)
미지수가 너무 많았기 때문이다,
따라서 우리는 Stress- strain 관계식 (8,9)를 대입할 것이다.
대입해주면,
![[Pulsatile flow] 2. Force on the elastic tube wall](./images/img-031.png)
약분하고 정리해주면,
...
![[Pulsatile flow] 2. Force on the elastic tube wall](./images/img-033.png)
다음포스터에서 뵙겠습니다
